...
namespace _2025.Day10
{
    public class Day10 : Day<List<(List<char>, List<List<int>>, List<int>)>, long, long>
    {
        ...
        {
            // I still considered a diagram which started with all switches Off '.'
            // Using Eric's explanation to reach desired joltages, I turned the switches on for each button press like Part 1
            // After solving example by hand I arrived at a conclusion that each odd joltage ended with On '#' and even with Off '.'
            // Example, machine 1: After  pressing (3) once, (1,3) three times, (2,3) three times, (0,2) once, and (0,1) twice - [....] --> [##.#] and acheived joltage {3,5,4,7}
            // using my Part 1, I will start with a diagram that would result in # for odd joltage and . for even joltage and calculate # of presses

            long sum = 0;
            //foreach (var machine in input)
            //{
            //    // use joltages to derive expected diagram
            //    var diagram = machine.Item3.Select(x => x % 2 == 0 ? Off : On).ToList();
            //    var buttons = machine.Item2;
            //    // PriorityQueue<expected, presses, buttons>
            //    PriorityQueue<(List<int>, List<List<int>>), int> queue = new PriorityQueue<(List<int>, List<List<int>>), int>();
            //    // lights initially on
            //    List<int> lights = new List<int>();
            //    for (int i = 0; i < diagram.Count; i++)
            //    {
            ...
            //                var newExp = button.Union(expected).Except(button.Intersect(expected)).Order().ToList();
            //                var newPri = priority + 1;
            //                var newPressed = pressed.ToList();
            //                newPressed.Add(button);
            //                queue.Enqueue((newExp, newPressed), newPri);
            //            }
            //        }
            //    }
            //    sum += presses;
